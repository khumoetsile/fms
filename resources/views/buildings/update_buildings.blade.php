...
<div class="page-wrapper">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="form-single-submit" action="{{ route('buildings.update', $building->id) }}" method="post" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-row">
                            <div class="col-md-2">
                            ...
                            <div class="col-md-4 mb-3">
                                <input type="text" maxlength="254" class="form-control form-control-sm @error('building_name') is-invalid @enderror" id="building_name" name="building_name" value="{{ $building->building_name }}" required>
                        ...
                        <div class="form-row">
                            ...
                            <div class="col-md-2">
                                <input type="hidden" name="submit_building" id="submit_building" value="1">
    ...
    <script>
        ...
        $('#building_code').keyup(function(){
        var building_code = $(this).val();
        var id_lenght= building_code.toString().length;
        if(id_lenght>3){
        $.ajax({
        type: "POST",
        url: "{{route('check.buildings')}}",
        data: {'building_code':building_code},
        dataType: 'json',
        success : function(data) {
        if(data.building_code)
             {
                 $('#presentreg').html('Opss.!..Building Code Already Exist');
                 $('#absentreg').html('');
             }else{

                $('#absentreg').html('Congrats!..Building Code is Available.');
                $('#presentreg').html('');
            }
        },
